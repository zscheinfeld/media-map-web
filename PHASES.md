...
**Progress:**
- ✅ **Ingest** (`jobs/ingest-valuations.ts`): `buildYears` + `yearlyBillions` (Oct-1 past / latest current). **Once a company has any value on the sheet, the daily run only ever writes its current-year column** — every past year is preserved verbatim (client can correct historical figures; edits are never overwritten; no re-pull on ticker change or rollover). A full history pull happens only to seed a brand-new/empty company; clearing an FMP company's year cells re-triggers it (the manual "refresh history" escape hatch). One-time legacy-month sweep (`clearTabValues`, preserves client Notes/vetting). `enforceNumberFormat`.
...
- ✅ **Appearance windows as year ranges** — `appearanceWindow` object now stores `start_year`/`end_year` (integers). Added `appearance_windows` to the **company** schema (was entity-only). Shared `yearWindowsActiveAt` in map-core filters companies + entities by the viewed year in both the public resolver and the Studio editor (empty = always visible). Public side deployed; the schema/editor side needs the Studio redeploy. *Note: the resolver hides out-of-window companies at the viewed year, so the aggregate view reflects companies visible at that year — per-year aggregate windowing is a possible follow-up.*

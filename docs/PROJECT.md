...
- **GOOGLEFINANCE currency switches** — ✅ handled (2026-09-21): the current-year formula computes both the local-currency and already-USD readings and keeps the one closer to last year's value (far-from-parity currencies only; EUR/GBP/CAD/AUD left alone). Set `Currency` to the company's HOME currency (Sony → JPY) so the two readings differ. LAUNCH_CHECKLIST.md → Roadmap — data quality.

...
- **Valuations** — a published **Google Sheet**, one column per **year**. The current-year column is populated by live **`GOOGLEFINANCE()`** formulas (converted to billions USD via an `FX_to_USD` column); past years are manual snapshots; a scheduled + Sanity-webhook **roster reconciler** keeps the sheet's structure in sync with Sanity. *(FMP was the prior source — retired as the live provider on 2026-09-08; see [GOOGLE_FINANCE.md](GOOGLE_FINANCE.md).)* No database — a sheet is robust at this scale (≈175 companies × ~12 years).
...
- **Appearance windows (companies + entities).** A `[start_year, end_year]` list; the node renders only on years inside a window (empty = always). Shared `yearWindowsActiveAt` enforces this in both the public resolver and the Studio editor. In the **aggregate view**, each company's bars appear only in its window years. **Retiring a company** (acquired / went private / shut down) is exactly this: keep its year values in the sheet, set `data_source` → manual, and cap its appearance window's `end_year` at its last year so it shows on old maps but vanishes from recent ones. Blanking cells alone won't hide it (no window = always visible). Client-facing recipe: [SHEET_EDITING_GUIDE.md → Retiring a company](SHEET_EDITING_GUIDE.md#retiring-a-company-acquired-went-private-shut-down).
...
| Data | Source |
|---|---|
| Public-company market caps (current year) | **Dynamic** — live `GOOGLEFINANCE()` formulas in the Google Sheet, keyed by the GF `ticker` + `exchange` + `Currency`/`FX_to_USD` (in billions USD). Past years are manual snapshots. |
| Company **data source** (which provider) | **Manual — Sanity** `data_source` reference (`api` → the reconciler writes the GF formula + syncs ticker/exchange/currency; `manual` → hand-entered, reconciler leaves the row's values alone). Mirrored into the sheet's "data source" column — edit it in Sanity, not the sheet. |
...
| **3 — In-Studio Map Editor** (full content model: companies, entities, sectors, connections, vitals, content, time-scoped layout knobs, de-overlap physics; sizes planets from the **live valuation sheet** — slug × year — at the editor's selected year, with red not-live labels; connected collapsible control panel) | ✅ done |
...
**Recently completed:**
- **FMP → Google Finance** ✅ *(live 2026-09-08; merged from `google-finance-alternative`)*. The paid FMP market-cap ingest is replaced with free `GOOGLEFINANCE()` formulas in the sheet (the app is unchanged — it just reads the published CSV). Live formulas for the current year; historical years manual (GF has no historical market cap). The re-tickering of OTC/ADR symbols to primary listings is done (Japanese via OTCMKTS+JPY, `Currency` override field). A scheduled + Sanity-webhook **roster reconciler** (`jobs/gf-sync-roster.ts`) keeps the sheet in sync. FMP is retained only as the tested rollback target. Full guide: [docs/GOOGLE_FINANCE.md](GOOGLE_FINANCE.md).

**Outstanding:**
- **Mobile + touch** — ✅ touch is implemented (one-finger pan, two-finger pinch-zoom, tap-to-focus); square-view mobile position authoring is live in the Map Editor. Remaining: real-device QA + edit-mode planet dragging on touch (starts from `onMouseDown`). See LAUNCH_CHECKLIST.md §2–3.
- **Dynamic news feed** *(targeted for launch)* — nightly Action pulls free RSS (Yahoo Finance by ticker / Google News fallback) → Sanity `external_articles` (schema + panel already exist); Eshap content stays manually curated. Short plan + open decisions in LAUNCH_CHECKLIST.md §4b.
- **About page** — ✅ built ([src/AboutModal.tsx](../src/AboutModal.tsx)): a CMS-driven modal (Sanity `about` singleton as a block/page-builder — sections = tabs, composed of header/body/button/link/photo blocks). Opened by the ABOUT (ⓘ) button; mobile-safe height + active tab stays left-aligned. Remaining: author content in Studio. LAUNCH_CHECKLIST.md §4b.
- **Historical map layout** — ✅ improved (2026-09-18): Time Machine "Explore" flies planets out from their sector wells; saved-year toggles morph between cached per-year layouts (no lag, no clustering/overlap). Remaining: adjust for mobile. LAUNCH_CHECKLIST.md §3.
- **Studio Map Editor polish** — ✅ typed X/Y positions for alignment, draggable Changes panel + collapsible inspector, draft/published duplicate fix. Studio redeployed 2026-09-18.
- **Typeface → ITC Franklin Gothic** — ✅ swapped globally from Calibri (Adobe Typekit `"franklin-gothic"` on-screen; self-hosted Libre Franklin for the PNG export). Pending Adobe-side: domain allowlist + activate Medium/Demi weights. See [CLAUDE.md → Typography](../CLAUDE.md).
- **Search** — ✅ built ([src/SearchBar.tsx](../src/SearchBar.tsx)): icon beside the view tabs expands over them; ranked dropdown, non-matches dim, and picking a result travels per view (Map zooms, Linear/List scroll, Aggregate pins). Remaining: QA on mobile. LAUNCH_CHECKLIST.md §4b.
- **Download: consistent framing + branding** — ✅ done (2026-09-18; [src/exportMap.tsx](../src/exportMap.tsx)): 16:9 PNG of the present map on a fixed canonical frame, side-panel legend + Eshap logo + Substack QR, a label-aware de-overlap pass, pre-rendered in the background so the download is instant. LAUNCH_CHECKLIST.md §4b.
- **Launch infra / SEO / analytics** *(targeted for launch)* — custom domain + HTTPS, real `<title>` (currently the repo name), proper favicon set, OG/social share meta + 1200×630 image, and analytics (GA4 vs a cookieless option — decision pending). LAUNCH_CHECKLIST.md §4c.
- **Front-end polish** — restyle toward the light mockup; loading-moment animation.
- ~~**Automate non-USD → USD market caps in the ingest**~~ *(OBSOLETE — the GF cut-over solved this in-sheet: the `FX_to_USD` column × `GOOGLEFINANCE("CURRENCY:xxxUSD")` converts every current-year cell to USD, and past years are entered pre-converted. Kept below only as reference if FMP is ever revived.)* FMP returns market cap in the stock's **listing currency**, not USD. Verified against the live Starter key: non-US **profile** (current cap) works (e.g. `RELIANCE.NS` → 17.3T INR, `PSM.DE` → 838M EUR), **forex history** works (`{CCY}USD` daily, direct — INRUSD/KRWUSD/EURUSD/HKDUSD/CNYUSD/JPYUSD/GBPUSD), but **historical market cap for non-US is 402-blocked**. Plan = **hybrid**: (a) prefer US-listed **ADR** tickers where a liquid one exists (BABA, TCEHY, TSM…) → FMP returns USD *and* full history, no FX; (b) for non-US primary listings, convert `localCap × {CCY}USD` in [jobs/ingest-valuations.ts](../jobs/ingest-valuations.ts) — current-year works **now** (profile × live rate), full history lights up after an FMP upgrade (historical rate at each Oct-1 snapshot); (c) private (ByteDance) stays manual. **Correctness musts:** use the **historical** FX rate at each snapshot's date (not today's) for frozen years; handle London **pence (`GBp`)** = GBPUSD ÷ 100; and a **guard** — if `currency ≠ USD` and no rate resolves, write `NA`, never the raw local number (this also fixes a latent bug: an api-sourced non-US ticker would otherwise write its local-currency cap straight into a USD cell). **Spot-check FMP's non-US caps** — `005930.KS` (Samsung) came back ~3× high. Integration points: capture `currency` in `fetchProfile` + a cached FX helper in [jobs/lib.ts](../jobs/lib.ts). Test in local CSV mode (`npm run ingest` → `jobs/market_cap.csv`) before it touches the live sheet.
- **Fix the resize problem** *(roadmap)* — resizing ratchets small planets outward and leaves gaps: `labelRadii` is the only physics input that changes on resize, and the re-settle is push-only (~140 separation sweeps vs ~15% attraction), so drift never reverses. Affects every re-settle trigger, not just resize. LAUNCH_CHECKLIST.md → Roadmap — layout stability.
- **GOOGLEFINANCE currency switches** — ✅ fixed for far-from-parity currencies (HKD, JPY, KRW, INR, CNY, SEK; 2026-09-21): the current-year formula computes both the local-currency and already-USD readings and keeps the one closer to last year's value. Verified live on Sega Sammy. Set `Currency` to the company's HOME currency (Sony → JPY, Alibaba → HKD) so the two readings differ. **Open decision** for EUR/GBP/CAD/AUD (~18 companies; Deutsche Telekom $151B is the one that matters): ratio-signature day-over-day correction vs. manual entry for the top 3 vs. accept. LAUNCH_CHECKLIST.md → Roadmap — data quality.
- **Plausibility check on Google Finance values** *(roadmap)* — GOOGLEFINANCE's `marketcap` currency can flip for dual-listed tickers (Sony went yen → dollars in one day, showing $0.90B); distrust GF-sourced values >~5× off the prior year, exempting manual rows. LAUNCH_CHECKLIST.md → Roadmap — data quality.
...
6. **Valuation provider** — FMP Starter covers US + **non-US profiles/forex** but **not** non-US *historical* market cap (402). Moving to a global plan (FMP Ultimate / EODHD) — which unlocks full non-US history and lets the USD auto-conversion above cover past years — is a cost decision to make with the client. See the "Automate non-USD → USD market caps" item under Build status → Outstanding.

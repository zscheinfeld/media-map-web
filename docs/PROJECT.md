...
- **About page** — ✅ built ([src/AboutModal.tsx](../src/AboutModal.tsx)): a CMS-driven modal (Sanity `about` singleton as a block/page-builder — sections = tabs, composed of header/body/button/link/photo blocks). Opened by the ABOUT (ⓘ) button. Remaining: author content in Studio. LAUNCH_CHECKLIST.md §4b.
...
- **Download: consistent framing + branding** *(targeted for launch)* — export a **fixed canonical frame** (full canvas, zoom-independent) instead of the current pan/zoom, and composite the ESHAP logo + a QR code onto the PNG. LAUNCH_CHECKLIST.md §4b.
